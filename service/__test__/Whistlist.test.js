...
describe('testing /wishlists', () => {
  beforeAll((done) => {
    seederUser()
      ...
      .then((users) => {
        ...
        (secondUserId = users[1].id), (secondUserAccess_token = generateToken(user));
        done();
      })
      .catch((err) => {
        done(err);
  ...
  // POST Gagal jika wishlists sudah ada
  describe('Failed POST request', () => {
    it('Should return response with status code 400, if already whislist same product', (done) => {
      const body = {
        ProductId,
      };
      request(app)
        ...
        .end((err, res) => {
          ...
          expect(typeof res.body).toEqual('object');
          expect(res.body).toHaveProperty('msg', 'You already wishlist this product');
  ...
  // GET Gagal
  describe('Failed GET request', () => {
    it('It should return status 401 if doesnt have access_token', (done) => {
      request(app)
        .get('/wishlists')
        .end((err, res) => {
          if (err) {
